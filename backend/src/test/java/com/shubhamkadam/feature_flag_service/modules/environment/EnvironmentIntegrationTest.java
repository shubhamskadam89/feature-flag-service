...
class EnvironmentIntegrationTest {
    ...
    void setUp() {
        ...
        adminUser = userRepository.save(
            User.builder().id(UUID.randomUUID()).name("Admin").email("admin@test.com").passwordHash("hash").build()
        );
        memberUser = userRepository.save(
            User.builder().id(UUID.randomUUID()).name("Member").email("member@test.com").passwordHash("hash").build()
        );
        ...
        otherOrg = organizationRepository.save(
            Organization.builder().id(UUID.randomUUID()).name("Other Org").createdBy(adminUser.getId()).build()
        );
    ...
    void testTenantIsolation_CannotCreateInOtherProject() throws Exception {
        ...
        mockMvc
            .perform(
                post("/api/v1/projects/{projectId}/environments", otherProject.getId())
                    .header("Authorization", "Bearer " + adminToken)
                    .header("organization-id", myOrg.getId().toString())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(request))
    ...
    void testEnvironmentCrudAndKeyRotation() throws Exception {
        ...
        String envId = objectMapper
            .readTree(createResult.getResponse().getContentAsString())
            .path("data")
            .path("id")
            .asText();
        String originalKey = objectMapper
            .readTree(createResult.getResponse().getContentAsString())
            .path("data")
            .path("plaintextApiKey")
            .asText();

        // Read List
        mockMvc
            .perform(
                get("/api/v1/projects/{projectId}/environments", myProject.getId())
                    .header("Authorization", "Bearer " + adminToken)
                    .header("organization-id", myOrg.getId().toString())
            )
            .andExpect(status().isOk())
        ...
        mockMvc
            .perform(
                patch("/api/v1/projects/{projectId}/environments/{envId}", myProject.getId(), envId)
                    .header("Authorization", "Bearer " + adminToken)
                    .header("organization-id", myOrg.getId().toString())
                    .contentType(MediaType.APPLICATION_JSON)
        ...
        // Rotate Key
        MvcResult rotateResult = mockMvc
            .perform(
                post("/api/v1/projects/{projectId}/environments/{envId}/rotate-key", myProject.getId(), envId)
                    .header("Authorization", "Bearer " + adminToken)
                    .header("organization-id", myOrg.getId().toString())
            )
            .andExpect(status().isOk())
        ...
        String newKey = objectMapper
            .readTree(rotateResult.getResponse().getContentAsString())
            .path("data")
            .path("plaintextApiKey")
            .asText();
        ...
        mockMvc
            .perform(
                delete("/api/v1/projects/{projectId}/environments/{envId}", myProject.getId(), envId)
                    .header("Authorization", "Bearer " + adminToken)
                    .header("organization-id", myOrg.getId().toString())
            )
            .andExpect(status().isOk());
        ...
        mockMvc
            .perform(
                get("/api/v1/projects/{projectId}/environments/{envId}", myProject.getId(), envId)
                    .header("Authorization", "Bearer " + adminToken)
                    .header("organization-id", myOrg.getId().toString())
            )
