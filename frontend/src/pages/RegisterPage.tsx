...
import { ArrowLeft, User, Mail, KeyRound, AlertTriangle, ArrowRight } from 'lucide-react';
...
export const RegisterPage: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col md:flex-row bg-[#fffdf6] text-[#131311] select-none font-display">
      ...
      {/* Left side: Editorial Cream registration form */}
      <div className="w-full md:w-1/2 min-h-screen flex flex-col justify-center px-8 md:px-20 py-20 relative bg-[#fffdf6] bg-grid-light">
        <div className="max-w-md w-full mx-auto space-y-8 relative z-10">
          ...
          <div className="space-y-2">
            <span className="font-mono text-[9px] font-bold text-[#131311] uppercase tracking-widest bg-[#131311]/5 px-2 py-0.5 rounded border border-[#131311]/15 inline-block">
              // REGISTER ACCOUNT
            </span>
            ...
            <p className="text-xs text-[#575755] font-medium leading-relaxed">
              Start setting up environments, flag segments, and target rules on our sub-0.05ms edge network.
            </p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[9px] font-mono font-bold text-[#575755] uppercase tracking-wider flex items-center gap-1.5">
                <User className="w-3 h-3 text-[#131311]" /> Full Name
              </label>
              <input
                type="text"
                required
            ...
            >
              <span>{loading ? 'CREATING ACCOUNT...' : 'PROCEED TO ACCOUNT'}</span>
          ...
          <div className="pt-4 border-t border-[#131311]/10 flex justify-between items-center text-xs font-mono relative">
            <span className="text-[#575755]">Already have an account?</span>
            <Link to="/login" className="font-bold underline hover:text-black">
              Log in instead →
            </Link>
            
            {/* Handwritten note next to link */}
            <span className="font-handwritten text-base text-emerald-800 absolute -bottom-6 -left-2 rotate-[2deg] hidden sm:inline-block">
              // Instant onboarding guarantee
            </span>
          </div>
